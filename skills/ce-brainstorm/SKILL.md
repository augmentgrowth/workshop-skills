...
metadata:
  origin:
    adapted_from: "Every - compound-engineering-plugin (ce-brainstorm)"
    ...
    note: >-
      ...
      outside that plugin. Body text is upstream's; references to plugin
      siblings that do not ship here (ce-pov, ce-doc-review, ce-proof, lfg) and
      to bundled scripts were softened to graceful absence. Not original work by
      Augment Growth. Upstream license text: see NOTICE.md at the repository
      root.
---
...
## Companion skills and graceful absence
...
Any *other* `ce-` skill named below (`ce-pov`, `ce-doc-review`, `ce-proof`, `ce-compound`, `lfg`, and so on) comes from the fuller upstream plugin and is **probably not installed**. Wherever one is named: check whether it is actually available, and if it is not, **do not stop and do not silently drop the step** — say in one line that the companion skill is not installed, do the closest thing you can do yourself in this session, and continue. Never offer the user a menu option that routes to a skill you have not confirmed is present.

**Where no shell tool is available** (for example Claude Desktop, which has no terminal), skip every shell block in this skill and its references. Each one is an optimization, not a gate: read files with the ordinary file tools, ask the user for a folder instead of resolving a scratch directory, and continue. Bundled helper scripts are not shipped with this copy — the text fallback documented beside each one is the path to take.
...
When the shape matches — at intake, or whenever later dialogue (Phases 1.3–2) clarifies a request into it — `ce-pov` would normally take the run. **`ce-pov` does not ship with this copy** and its routing reference is not bundled here. Handle it inline instead: say in one line that this is a verdict question rather than a scoping question, then either (a) give the verdict yourself — name the candidate, weigh it against this specific project's constraints, and commit to a graded recommendation with the reasoning that drove it — or (b) ask whether the user would rather scope the work assuming the candidate is adopted, and continue the normal workflow from their answer. Never silently switch; never pretend a skill you do not have is running.
...
*Topic Scan (grounding scout)* — Create and retain the absolute scratch directory with the shell block below, substituting the absolute path of this skill's directory and a short unique run slug. **If no shell tool is available, skip the scratch directory entirely**: run the scout's search inline instead and keep its findings in the conversation as the gist, with no dossier file and no dossier path to pass downstream.
...
**Model elevation.** Upstream dispatches approach generation to a user-chosen model through a bundled script. **That reference and its script are not shipped here** — generate approaches with your own session model instead. If the user names a model they would rather use for this step, tell them it is not wired up in this copy and proceed with your own reasoning rather than leaving the step undone.
