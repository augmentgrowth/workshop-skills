...
**Availability gate for this menu (read first).** Only `ce-plan`, `ce-work`, and `ce-handoff` ship alongside this skill. `lfg`, `ce-doc-review`, and `ce-proof` are upstream-plugin siblings that are **probably not installed**. Before rendering the menu, drop any option whose skill you have not confirmed is available, and renumber what remains. Concretely, unless you confirm otherwise:

- **Option 2 (`lfg`)** — hide it. If the user asks for hands-off shipping anyway, say `lfg` is not installed and offer option 1 (`ce-plan`) followed by `ce-work` as the two-step equivalent.
- **Option 3 (`ce-doc-review`)** — replace it with **"Pressure-test the requirements"** performed inline: reread the artifact yourself against the dialogue looking for gaps, conflicts, weak premises, and scope creep; report what you found; apply the fixes the user approves. Say plainly that this is your own pass, not the dedicated review skill.
- **Option 4 (Publish to Proof)** — hide it. If the user wants to share the document, offer the file path, or whatever sharing capability this platform actually exposes.

Every reference below to those three skills is subject to this gate. Do not tell a user a skill will run when it will not.
