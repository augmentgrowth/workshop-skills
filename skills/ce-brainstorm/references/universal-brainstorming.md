...
- **Save summary to disk** → write the summary as a markdown file, named `YYYY-MM-DD-<short-topic>-brainstorm.md`. Save it **beside the source material the brainstorm drew on** — the project folder holding the documents, notes, or transcript that were discussed. Fall back to the current working directory only when the brainstorm drew on no files, and say which folder you used either way. If the right folder is genuinely ambiguous, ask before writing rather than dropping a stray file at the workspace root.
- **Publish to Proof — shareable link** → *(hide this option unless `ce-proof` is confirmed installed — it is an upstream-plugin sibling that does not ship here. Where it is absent, offer the file path or whatever sharing capability this platform exposes instead.)* Load the `ce-proof` skill to publish the doc to Every's Proof editor and get a shareable link to read, comment on, or share with others (one-way; the local summary stays canonical)
