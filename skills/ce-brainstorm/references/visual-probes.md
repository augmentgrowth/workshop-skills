...
**Not shipped in this copy.** The bundled `visual-probe-server.js` helper is part of the fuller upstream plugin and is not included here, and Claude Desktop has no terminal to run it in regardless. Treat the helper section below as unavailable: use the text path documented above, or render the sketch inline in the conversation where the platform can show it. Everything else in this reference — when to offer a visual probe, what to build, what to label as directional — applies unchanged.

The upstream helper instructions are retained below for anyone installing this skill alongside the full plugin.
