...
metadata:
  origin:
    adapted_from: "Every - compound-engineering-plugin (ce-handoff)"
    ...
    note: >-
      ...
      outside that plugin. The body is upstream's; the shell-dependent default
      storage path was softened so the skill degrades gracefully where no
      terminal is available. Not original work by Augment Growth. Upstream
      license text: see NOTICE.md at the repository root.
---
...
**Environment note.** This skill ships standalone. Where a shell is unavailable (for example Claude Desktop, which has no terminal), skip the shell blocks below and use the ordinary file tools instead: ask the user where to save the handoff, or save it beside the work it describes. Everything else in this skill works unchanged.
...
When the user did not choose another destination, resolve the managed root with the shell block below. **If no shell tool is available, do not attempt it** — instead ask the user for a folder (or use the current working folder), save the handoff there as a Markdown file, and tell them where it landed. The frontmatter and body contracts below still apply.
...
1. Search the folder or collection the user supplied; otherwise run the managed-root block above in the current shell call and enumerate candidate files beneath `$SCRATCH_ROOT/ce-handoff/`. Where no shell is available, ask the user which folder to search instead of guessing, then enumerate it with the ordinary file tools. Bound the candidate set before inspecting content; prefer recent files and current repository or working-directory affinity without making repository affinity mandatory.
