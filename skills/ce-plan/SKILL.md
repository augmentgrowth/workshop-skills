...
metadata:
  origin:
    adapted_from: "Every - compound-engineering-plugin (ce-plan)"
    ...
    note: >-
      ...
      siblings that do not ship here (ce-doc-review, ce-proof, ce-debug, lfg)
      and to bundled scripts were softened to graceful absence. Not original
      work by Augment Growth. Upstream license text: see NOTICE.md at the
      repository root.
---
...
Any *other* `ce-` skill named below (`ce-doc-review`, `ce-proof`, `ce-debug`, `ce-compound`, `lfg`, and so on) comes from the fuller upstream plugin and is **probably not installed**. Wherever one is named: check whether it is actually available, and if it is not, **do not stop and do not silently drop the step** — say in one line that the companion skill is not installed, do the closest thing you can do yourself in this session, and continue. Never offer the user a menu option that routes to a skill you have not confirmed is present. Where a step is marked mandatory or non-optional upstream (most notably the Phase 5.3.8 document review), the *intent* is mandatory, not the specific skill: perform the pass yourself and say that you did.

**Where no shell tool is available** (for example Claude Desktop, which has no terminal), skip every shell block in this skill and its references — config reads, `git` probes, clipboard copies. Each one is an optimization, not a gate: fall through to the documented defaults, read files with the ordinary file tools, and continue.
...
**Model elevation.** Upstream dispatches the interpret-findings-then-author step to a user-chosen model through a bundled script. **That reference and its script are not shipped here** — author the plan with your own session model instead. If the user names a model they would rather use for this step, tell them it is not wired up in this copy and proceed with your own reasoning rather than leaving the step undone.
