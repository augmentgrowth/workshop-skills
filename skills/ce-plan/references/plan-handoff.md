...
**Availability gate for everything below (read first).** Only `ce-plan`, `ce-brainstorm`, `ce-work`, and `ce-handoff` ship alongside this skill. `ce-doc-review` and `ce-proof` are upstream-plugin siblings that are **probably not installed**, and `/goal` and issue-tracker CLIs need a harness or terminal that may not exist here. Before acting on any instruction below, confirm the capability exists; where it does not:

- **5.3.8 document review (`ce-doc-review`)** — the review is still mandatory, the *skill* is not. Perform the pass yourself: reread the written plan for coherence, feasibility, scope alignment, contradictions, and thin sections; apply the safe mechanical fixes silently; surface the judgment calls above the menu the way the headless envelope would. Summarize it as your own pass, not as `ce-doc-review` output, and record `fixes_applied` / remaining findings from what you actually did.
- **Option 2 (`/goal`)** — render only where the host genuinely exposes goal capability. On Claude Desktop and anywhere else without it, hide the option. The clipboard shell block below is likewise skippable: where no shell exists, just print the prompt.
- **Option 5 (Publish to Proof)** — hide it unless `ce-proof` is confirmed present. If the user wants to share the plan, offer the file path, or whatever sharing capability this platform actually exposes.
- **Option 4 (Create Issue)** — keep it only when a tracker interface (connector, MCP tool, or authenticated CLI) is actually reachable. Where none is, say so and offer to write the issue text into the conversation instead.

Renumber the visible options after dropping any. Do not tell a user a skill will run when it will not.
