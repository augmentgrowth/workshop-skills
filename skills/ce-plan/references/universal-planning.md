...
2. **Publish to Proof — shareable link** — *(hide this option unless `ce-proof` is confirmed installed — it is an upstream-plugin sibling that does not ship here. Where it is absent, offer the file path or whatever sharing capability this platform exposes instead.)* Publish the doc to Every's Proof editor and get a shareable link to read, comment on, or share with others. Load the `ce-proof` skill to create the shared document and return the URL. One-way: nothing syncs back to disk.
