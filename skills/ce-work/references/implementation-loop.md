...
**Cross-model execution is not shipped in this copy** (see the SKILL.md section "What ships here, and what does not"), so this loop always runs against native inline/subagent workers. It owns unit ordering, evidence selection, actual-scope inspection, authoritative verification, and incremental canonical commits throughout. Ignore any upstream instruction elsewhere that defers worker authoring to an external controller protocol — there is no controller here.
