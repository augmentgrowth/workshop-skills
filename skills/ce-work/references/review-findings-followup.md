...
**Load this reference only if `ce-code-review` actually ran.** It is an upstream-plugin sibling that is probably not installed alongside this copy. Where it is absent, `shipping-workflow.md`'s availability gate has you perform the review yourself — in that case there is no `mode:agent` JSON envelope to filter on, so skip this reference entirely and act on your own findings directly: batch them by file, fix them, run what verification you can, and commit.
