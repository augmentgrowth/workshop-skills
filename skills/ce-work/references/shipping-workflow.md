...
**Availability gate for everything below (read first).** `ce-code-review`, `ce-simplify-code`, `ce-commit-push-pr`, and `ce-worktree` are upstream-plugin siblings that are **probably not installed** alongside this copy, and this whole workflow assumes a shell with `git` and a working test runner. Before acting on any instruction below, confirm the capability exists; where it does not:

- **`ce-code-review`** — the review is still required, the *skill* is not. Read the full diff yourself against the plan's scope and the project's conventions, looking for correctness bugs, regressions, missing tests, and standards violations; report findings the same way; then apply the fixes. Say it was your own pass, not the review skill's. The `mode:agent` JSON contract and the findings-followup reference then do not apply — handle the findings directly.
- **`ce-simplify-code`** — review the changed files yourself for reuse and consolidation, exactly as the upstream fallback already says.
- **`ce-commit-push-pr`** — write the commit and PR body yourself using the project's conventions.
- **`ce-worktree`** — use the harness's own isolation, or work in the current checkout on a named branch.
- **No shell / no test runner** (for example Claude Desktop, which has no terminal) — do not fake a passing test run, a commit, or a pushed branch. Say which gates could not run, do the review pass you *can* do by reading the code, and report the change set as unverified.

Do not tell a user a skill will run when it will not, and never record a gate as passed when it did not execute.
