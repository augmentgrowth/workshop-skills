...
This reference covers how residual actionable findings are filed in the project's tracker. Loaded by the `ce-work` Residual Work Gate.

**Requires a reachable tracker.** Filing needs a connector, MCP tool, or authenticated CLI, and some paths below need a shell. Where none is reachable — including on Claude Desktop, which has no terminal — do not silently drop the residuals: list them in the conversation, say they could not be filed and why, and ask the user where they want them recorded.
